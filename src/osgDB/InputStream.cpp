...
{
    osg::Quat::value_type x, y, z, w; *this >> x >> y >> z >> w; //hxx double--> osg::Quat::value_type 
    q.set( x, y, z, w ); return *this;
}

InputStream& InputStream::operator>>( osg::Plane& p )
{
    osg::Plane::value_type p0, p1, p2, p3; *this >> p0 >> p1 >> p2 >> p3; //hxx double--> osg::Plane::value_type
...
//add in 2021 07 28 hxx
InputStream& InputStream::operator>>(osg::Vec2i64& v) { *this >> v.x() >> v.y(); return *this; }
InputStream& InputStream::operator>>(osg::Vec2ui64& v) { *this >> v.x() >> v.y(); return *this; }
InputStream& InputStream::operator>>(osg::Vec3i64& v) { *this >> v.x() >> v.y() >> v.z(); return *this; }
InputStream& InputStream::operator>>(osg::Vec3ui64& v) { *this >> v.x() >> v.y() >> v.z(); return *this; }
InputStream& InputStream::operator>>(osg::Vec4i64& v) { *this >> v.x() >> v.y() >> v.z() >> v.w(); return *this; }
InputStream& InputStream::operator>>(osg::Vec4ui64& v) { *this >> v.x() >> v.y() >> v.z() >> v.w(); return *this; }
...
osg::ref_ptr<osg::Array> InputStream::readArray()
{
    ...
    *this >> type;
    ...
    case ID_VEC4UI_ARRAY:
        ...
        break;
    //below is the extension for int64 and uint64 array hxx 2021 07 28
    case  ID_UINT64_ARRAY:
    {
        osg::UInt64Array* va = new osg::UInt64Array;
		readArrayImplementation(va, 1, INT64_SIZE);
        array = va;
    }
    break;
	case ID_INT64_ARRAY:
	{
		osg::Int64Array* va = new osg::Int64Array;
		readArrayImplementation(va, 1, INT64_SIZE);
		array = va;
	}
	break;
	//below is the extension for vec int64 and uint64 array hxx 2021 07 28
    case ID_VEC2I64_ARRAY:
    {
        osg::Vec2i64Array* va = new osg::Vec2i64Array;
		readArrayImplementation(va, 2, INT64_SIZE);
    }
    break;
	case ID_VEC2UI64_ARRAY:
	{
		osg::Vec2ui64Array* va = new osg::Vec2ui64Array;
		readArrayImplementation(va, 2, INT64_SIZE);
	}
	break;
	case ID_VEC3I64_ARRAY:
	{
		osg::Vec3i64Array* va = new osg::Vec3i64Array;
		readArrayImplementation(va, 3, INT64_SIZE);
	}
	break;
	case ID_VEC3UI64_ARRAY:
	{
		osg::Vec3ui64Array* va = new osg::Vec3ui64Array;
		readArrayImplementation(va, 3, INT64_SIZE);
	}
	break;
	case ID_VEC4I64_ARRAY:
	{
		osg::Vec4i64Array* va = new osg::Vec4i64Array;
		readArrayImplementation(va, 4, INT64_SIZE);
	}
	break;
	case ID_VEC4UI64_ARRAY:
	{
		osg::Vec4ui64Array* va = new osg::Vec4ui64Array;
		readArrayImplementation(va, 4, INT64_SIZE);
	}
	break;
	//below is the extension for MatrixfArray and MatrixdArray 2021 07 28 hxx
	case ID_MATRIXF_ARRAY:
	{
		osg::MatrixfArray* va = new osg::MatrixfArray;
		readArrayImplementation(va, 16, FLOAT_SIZE);
		array = va;
	}
	break;
	case ID_MATRIXD_ARRAY:
	{
		osg::MatrixdArray* va = new osg::MatrixdArray;
		readArrayImplementation(va, 16, DOUBLE_SIZE);
		array = va;
	}
	break;
	//below is the extension for quatArray 2021 07 28 hxx
    case  ID_QUAT_ARRAY:
    {
        osg::QuatArray* va = new osg::QuatArray;
        readArrayImplementation(va, 4, sizeof(osg::Quat::value_type));
        array = va;
    }
    break;
    default:
