...
public:
	...
	//add by myself 2021 07 28
	virtual void readInt64(int64_t& ll)
	{
		std::string str; if (prepareStream()) _sstream >> str; ll = std::stoll(str.c_str(), NULL, 0);
	}

	virtual void readUInt64(uint64_t& ull)
	{
		std::string str; if (prepareStream()) _sstream >> str; ull = std::stoul(str.c_str(), NULL, 0);
	}
